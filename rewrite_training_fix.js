...
// Find the line that starts with "    return ("
let returnLineIndex = -1;
for (let i = 0; i < lines.length; i++) {
    if (lines[i].startsWith('    return (')) {
        // Double check this is the main return
        if (i > 600 && i < 650) {
            returnLineIndex = i;
            break;
        }
...
// Slice off the end
const topHalf = lines.slice(0, returnLineIndex).join('\n');
...
const finalFileContent = topHalf + '\n' + newReturn;
fs.writeFileSync(file, finalFileContent, 'utf8');
console.log('Successfully replaced Training page render method accurately.');
